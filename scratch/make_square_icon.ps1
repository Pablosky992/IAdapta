...
# 1. Crop the house region (X=70, Y=80, Width=830, Height=400)
$rect = New-Object System.Drawing.Rectangle(70, 80, 830, 400)
$cropBmp = $srcBmp.Clone($rect, $srcBmp.PixelFormat)

# 2. Process cropBmp to make its cream background transparent using soft chroma-keying
...
for ($y = 0; $y -lt $cropBmp.Height; $y++) {
    for ($x = 0; $x -lt $cropBmp.Width; $x++) {
        ...
        if ($dist -lt $minDist) {
            ...
            # Keep original colors, just scale alpha
...
# 3. Create the 512x512 canvas filled with solid #F2F0E9 (R=242, G=240, B=233)
...
# Centered size of the house outline (Width=420, Height=202)
$destWidth = 420
$destHeight = 202
