export default async function handler(req, res) {
  ...
  try {
    ...
    if (successfulData) {
      return res.status(200).json({ text: successfulData.candidates[0].content.parts[0].text });
    } else {
      return res.status(500).json({ 
        error: 'Ningún modelo de IA está respondiendo correctamente. ' + (lastError?.message || ''),
        details: lastError 
      });
    }
  } catch (error) {
