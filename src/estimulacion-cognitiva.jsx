const { Icons, Navbar, Footer, CookieBanner } = window;
            ...
            <div className="max-w-5xl mx-auto">
              <div className="text-center mb-16">
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-10 mb-16">
                  ...
                  {/* Game 9 Teaser: Secuencia de Colores */}
                  <div 
                    onClick={() => navigateTo('simon')}
                    className="bg-white rounded-[2.5rem] overflow-hidden border border-rose-50 shadow-xl hover:shadow-2xl transition-all group flex flex-col cursor-pointer"
                  >
                    <div className="relative h-56 overflow-hidden">
                      <img src="simon_game_thumbnail.png" alt="Secuencia de Colores" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                      <div className="absolute inset-0 bg-gradient-to-t from-rose-950/60 to-transparent"></div>
                      <div className="absolute bottom-4 left-6 flex items-center gap-3">
                        <div className="w-12 h-12 bg-rose-600 text-white rounded-xl flex items-center justify-center text-2xl shadow-lg border border-rose-400/30">
                          <Icons.Brain />
                        </div>
                        <span className="text-white font-bold text-lg">Memoria de Secuencia</span>
                      </div>
                    </div>
                    <div className="p-8 flex flex-col flex-1">
                      <h3 className="font-display text-2xl font-bold text-brand-900 mb-4">Secuencia de Colores</h3>
                      <p className="text-gray-600 text-lg mb-8 flex-1 leading-relaxed">
                        Observa la secuencia de luces y sonidos y repítela en el mismo orden exacto. Entrena la atención y la memoria de trabajo.
                      </p>
                      <button className="w-full py-4 bg-rose-900 text-white rounded-2xl font-bold text-lg group-hover:bg-rose-800 transition-all flex items-center justify-center gap-3 shadow-lg shadow-rose-900/20">
                        Jugar ahora
                        <Icons.ArrowRight />
    ...
    const SectionDailyChallenge = function SectionDailyChallenge({ isStandalone, navigateTo }) {
      ...
      const SimonSubGame = ({ onComplete }) => {
        ...
        return (
          <div className="bg-white rounded-[3rem] p-8 shadow-2xl border-8 border-brand-100 anim-scale-in max-w-lg mx-auto w-full text-center">
            <span className="text-brand-400 font-bold uppercase tracking-widest text-sm mb-2 block">Reto: Secuencia de Colores</span>
            <h3 className="text-xl font-bold text-brand-900 mb-4">Repite el patrón de 4 colores</h3>

            <div className="mb-6">
              {isSuccess ? (
                <div className="py-2 px-6 bg-emerald-500 text-white rounded-full font-bold text-base inline-flex items-center gap-2">
                  <span>¡Secuencia Correcta!</span> ✨
                </div>
              ) : isSimonTurn ? (
                <div className="inline-flex items-center gap-2 bg-brand-900 text-white px-5 py-2 rounded-full text-xs font-bold animate-pulse">
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-ping"></span>
                  <span>Memoriza la secuencia...</span>
                </div>
              ) : (
                <div className="inline-flex items-center gap-2 bg-emerald-500 text-white px-5 py-2 rounded-full text-xs font-bold">
                  <span>¡Tu turno! ({userIndex}/4)</span>
                </div>
              )}
            </div>

            <div className="relative w-56 h-56 mx-auto grid grid-cols-2 gap-3 p-3 bg-brand-900 rounded-full shadow-xl border-4 border-brand-800">
              {SIMON_PADS.map((pad) => {
      ...
      const renderCurrentGame = (gameId, onComplete) => {
        switch(gameId) {
          ...
          case 'simon': return <SimonSubGame onComplete={onComplete} />;
          default: return null;
        }
      };
                ...
                <div className="grid grid-cols-3 gap-4 mb-10">
                  {dailyGames.map((g, i) => (
                    <div key={i} className="bg-brand-50 p-4 rounded-2xl border border-brand-100">
                      ...
                      <p className="text-xs font-bold text-brand-900">{g === 'math' ? 'Cálculo' : g === 'order' ? 'Orden' : g === 'visual' ? 'Agudeza' : g === 'memory' ? 'Memoria' : g === 'wordsearch' ? 'Rastreo' : g === 'intruder' ? 'Intruso' : g === 'wordbuilder' ? 'Palabras' : g === 'simon' ? 'Secuencia' : 'Juego'}</p>
                    </div>
                  ))}
                </div>
    ...
    const SIMON_LEVELS = [
      { id: 1, name: 'Fácil', desc: 'Secuencia pausada (hasta 7 pasos)', speed: 750, targetRounds: 7, isInfinite: false },
      { id: 2, name: 'Medio', desc: 'Velocidad estándar (hasta 10 pasos)', speed: 500, targetRounds: 10, isInfinite: false },
      { id: 3, name: 'Difícil', desc: 'Secuencia rápida (hasta 14 pasos)', speed: 350, targetRounds: 14, isInfinite: false },
      { id: 4, name: 'Infinito ♾️', desc: '¡Sin límite de pasos! Aumenta velocidad', speed: 650, targetRounds: Infinity, isInfinite: true }
    ];

    const SIMON_PADS = [
      { id: 0, name: 'Rojo', color: 'rose', bgNormal: 'bg-rose-500 hover:bg-rose-600 active:bg-rose-400 border-rose-600', bgActive: 'bg-rose-300 ring-8 ring-rose-200 scale-105 shadow-[0_0_35px_rgba(244,63,94,0.8)] z-10', freq: 261.63 },
      { id: 1, name: 'Verde', color: 'emerald', bgNormal: 'bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-400 border-emerald-600', bgActive: 'bg-emerald-300 ring-8 ring-emerald-200 scale-105 shadow-[0_0_35px_rgba(16,185,129,0.8)] z-10', freq: 329.63 },
      { id: 2, name: 'Azul', color: 'sky', bgNormal: 'bg-sky-500 hover:bg-sky-600 active:bg-sky-400 border-sky-600', bgActive: 'bg-sky-300 ring-8 ring-sky-200 scale-105 shadow-[0_0_35px_rgba(14,165,233,0.8)] z-10', freq: 392.00 },
      { id: 3, name: 'Amarillo', color: 'amber', bgNormal: 'bg-amber-400 hover:bg-amber-500 active:bg-amber-300 border-amber-500', bgActive: 'bg-amber-200 ring-8 ring-amber-100 scale-105 shadow-[0_0_35px_rgba(251,191,36,0.8)] z-10', freq: 523.25 }
    ];
    ...
    const SectionSimonGame = function SectionSimonGame({ isStandalone, navigateTo }) {
      ...
      const handlePadClick = (padId) => {
        ...
        if (padId === sequence[userIndex]) {
          ...
          if (currentLevel.isInfinite) {
            ...
            let s = 3;
            if (stepsCompleted >= 15) s = 5;
            else if (stepsCompleted >= 10) s = 4;
            else if (stepsCompleted < 5) s = 2;
            setStars(s);
          }
        }
      };
      ...
      return (
        <section className={`pt-36 pb-20 px-4 min-h-screen bg-brand-50 flex flex-col items-center ${isStandalone ? 'pt-36' : ''}`}>
          <div className="max-w-4xl w-full">
            ...
            {!gameStarted ? (
              <div className="max-w-2xl mx-auto">
                <h2 className="font-display text-4xl font-bold text-brand-900 mb-8 text-center">Secuencia de Colores</h2>
                <div className="bg-white rounded-[3rem] border border-brand-100 shadow-2xl p-10 anim-scale-in text-center">
                  <div className="text-6xl mb-6">🔔</div>
                  <h3 className="text-2xl font-bold text-brand-900 mb-4">Selecciona la dificultad</h3>
                  <p className="text-gray-600 mb-8 text-lg">Memoriza el patrón de luces y sonidos y repítelo en el mismo orden exacto.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {SIMON_LEVELS.map((lvl, idx) => (
                      <button
                        key={idx}
                        onClick={() => startGame(idx)}
                      ...
                      >
                        <span className="text-brand-900 font-bold text-xl">{lvl.name}</span>
                        <span className="text-brand-500 text-xs font-medium">{lvl.desc}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="max-w-xl mx-auto w-full">
                <GameHeader 
                  ...
                  subtitle={(
                    <>
                      <span className="bg-brand-900 text-white px-4 py-1 rounded-full text-sm font-bold uppercase tracking-widest">Nivel: {currentLevel.name}</span>
                      <span className="text-brand-600 font-bold uppercase tracking-widest">
                ...
                {gameStatus === 'playing' ? (
                  <div className="bg-white rounded-[3rem] border border-brand-100 shadow-2xl p-8 text-center anim-scale-in">
                    
                    <div className="mb-8">
                      {isSimonTurn ? (
                        <div className="inline-flex items-center gap-3 bg-brand-900 text-white px-6 py-2.5 rounded-full text-sm font-bold animate-pulse shadow-md">
                          <span className="w-3 h-3 rounded-full bg-amber-400 animate-ping"></span>
                          <span>Memoriza la secuencia de colores...</span>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-3 bg-emerald-500 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-md shadow-emerald-500/20">
                          <span>¡Tu turno! Repite la secuencia ({userIndex}/{sequence.length})</span>
                        </div>
                      )}
                    </div>

                    <div className="relative w-64 h-64 sm:w-80 sm:h-80 mx-auto grid grid-cols-2 gap-4 p-4 bg-brand-900 rounded-full shadow-2xl border-8 border-brand-800">
                      {SIMON_PADS.map((pad) => {
                        const isActive = activePad === pad.id;
                        return (
                          <button
                      ...
                      <div className="absolute inset-0 m-auto w-24 h-24 bg-brand-900 border-4 border-brand-700 rounded-full flex flex-col items-center justify-center shadow-xl z-20 pointer-events-none">
                        <span className="text-white font-display font-bold text-lg">{roundCount}</span>
                        <span className="text-[10px] text-brand-300 uppercase tracking-widest font-bold">Pasos</span>
                      </div>
                    </div>

                  </div>
                ) : gameStatus === 'won' ? (
                  <div className="bg-white rounded-[3rem] p-10 shadow-2xl border border-brand-100 text-center anim-scale-in">
                    <div className="text-6xl mb-6">🏆</div>
                    <h3 className="text-3xl font-bold text-brand-900 mb-2">¡Memoria Prodigiosa!</h3>
                    <p className="text-brand-500 font-bold uppercase tracking-widest mb-6">Has superado los {currentLevel.targetRounds} pasos en nivel {currentLevel.name}</p>
                    
                    <StarRating stars={stars} />

                    <div className="bg-brand-50 p-6 rounded-2xl mb-8 flex justify-around">
                      <div>
                        <p className="text-brand-400 text-xs font-bold uppercase mb-1">Tiempo Total</p>
                        <p className="text-3xl font-bold text-brand-900">{finalTime}s</p>
                      </div>
                      <div className="border-l border-brand-200 h-12 my-auto"></div>
                      <div>
                        <p className="text-brand-400 text-xs font-bold uppercase mb-1">Secuencia</p>
                        <p className="text-3xl font-bold text-brand-900">{sequence.length} pasos</p>
                      </div>
                    </div>

                    <ShareButtons game="Secuencia de Colores" time={finalTime} />

                    <div className="flex flex-col gap-3 mt-8">
                      <button onClick={nextLevel} className="w-full py-4 bg-brand-900 text-white rounded-2xl font-bold text-lg hover:bg-brand-800 transition-all shadow-xl shadow-brand-900/20">
                        {levelIdx < SIMON_LEVELS.length - 1 ? 'Siguiente Dificultad' : 'Volver a Jugar'}
                      </button>
                      <button onClick={() => setGameStarted(false)} className="w-full py-3 text-brand-600 font-bold hover:bg-brand-50 rounded-xl transition-all">
                        Cambiar Dificultad
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-[3rem] p-10 shadow-2xl border border-brand-100 text-center anim-scale-in">
                    <div className="text-6xl mb-6">{currentLevel.isInfinite && isNewRecord ? '🎉🏆' : '💥'}</div>
                    <h3 className="text-3xl font-bold text-brand-900 mb-2">
                      {currentLevel.isInfinite 
                        ? (isNewRecord ? '¡NUEVO RÉCORD PERSONAL!' : '¡Excelente Intento!') 
                    ...
                    <p className="text-gray-600 mb-6">
                      Alcanzaste <strong className="text-brand-900 font-bold">{sequence.length - 1} pasos consecutivos</strong> en esta partida.
                    </p>

                    {currentLevel.isInfinite && (
                      <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl mb-8 flex justify-around">
                        <div>
                          <p className="text-amber-800 text-xs font-bold uppercase mb-1">Pasos Logrados</p>
                          <p className="text-3xl font-bold text-amber-900">{sequence.length - 1}</p>
                        </div>
                        <div className="border-l border-amber-300 h-12 my-auto"></div>
                        <div>
                          <p className="text-amber-800 text-xs font-bold uppercase mb-1">Récord Máximo</p>
                          <p className="text-3xl font-bold text-amber-900">{localStorage.getItem('simon_infinite_record') || 0}</p>
                        </div>
                      </div>
                    )}

                    <div className="flex flex-col gap-3 mt-8">
                      <button onClick={() => startGame(levelIdx)} className="w-full py-4 bg-brand-900 text-white rounded-2xl font-bold text-lg hover:bg-brand-800 transition-all shadow-xl shadow-brand-900/20">
                        {currentLevel.isInfinite ? 'Jugar Modo Infinito de Nuevo' : 'Reintentar'}
                      </button>
                      <button onClick={() => setGameStarted(false)} className="w-full py-3 text-brand-600 font-bold hover:bg-brand-50 rounded-xl transition-all">
                        Cambiar Dificultad
                      </button>
    ...
    function App() {
      ...
      const syncPageFromUrl = useCallback(() => {
        ...
        else if (pageParam === 'simon') newPage = 'simon';
      ...
      return (
        <>
          ...
          <main id="main-content">
            ...
            {currentPage === 'simon' && (
              <SectionSimonGame isStandalone={isStandalone} navigateTo={navigateTo} />
            )}
